...
const SimilarityConfirmModal = ({ show, comment, onCancel, onConfirm }) => {
  if (!show) return null;

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg p-6 max-w-md w-full">
        <h2 className="text-xl font-bold mb-4 text-gray-800">Similar Comment Detected</h2>
        <p className="text-gray-700 mb-4">
          A similar comment was already posted:
        </p>
        <div className="bg-gray-100 p-3 rounded text-sm text-gray-600 mb-6">
          "{comment?.content}"
        </div>
        <div className="flex justify-end space-x-4">
          ...
          <button
            onClick={onConfirm}
            className="px-4 py-2 bg-[#3395ff] text-white rounded hover:bg-[#2678cc]"
          >
            Submit Anyway
          </button>
        </div>
