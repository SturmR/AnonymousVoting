...
function PickATime() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      {
        label: 'Admin Link',
        url:   `${window.location.origin}/rooms/${createdRoomId}/vote?user=${adminId}`
      },
      ...voterData.map(u => ({
        label: u.email,
        ...
        <div className="max-w-6xl w-full border border-dashed border-gray-300 rounded-lg flex flex-col md:flex-row">
          ...
          <div className="w-full md:w-1/3 p-8">
            ...
            <div style={{ 
              ...
              All features except start and end for time options can be changed afterwards. <br /> You can also disable any of the autogenerated time slots. 
            </div>                     
          </div>
        </div>
        </main>
